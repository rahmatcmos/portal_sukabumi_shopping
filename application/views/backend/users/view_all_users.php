    ...
    var ids=new Array()
     ...
     if(a>0){
       if(confirm("Are you sure want to publish?")){
        $.post("<?php echo base_url(); ?>backend/users/publish_users/yes/",{checkid:ids});
    ...
    var ids=new Array()
     ...
     if(a>0){
       if(confirm("Are you sure want to unpublish?")){
        $.post("<?php echo base_url(); ?>backend/users/publish_users/no/",{checkid:ids});
        setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
       }
      }else{
      alert('No data has choosen!');
    ...
    var ids=new Array()
     ...
     if(a>0){
       if(confirm("Are you sure want to Delete?")){
        $.post("<?php echo base_url(); ?>backend/users/delete_data_users/all/",{checkid:ids});
        setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
       }
      }else{
      alert('No data has choosen!');
...
<div id="main-container">
	<div id="breadcrumb">
		<ul class="breadcrumb">
			...
			<li>users</li>	 
			<li class="active">Data Users users</li>	 
		</ul>
	</div>
		...
		<div class="panel panel-default table-responsive">
			<div class="panel-heading">
				...
				<div class="btn-group pull-right">
					<a href="<?php echo base_url('backend/users/add_users') ?>"><button class="btn btn-info">Create New</button></a>
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table" class="table table-striped table-hover tinytable">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<?php if($row->active==0){ ?>
                <a href="<?php echo base_url().'backend/users/publish_users/yes/'.$id; ?>" onclick="return confirm('Do you want to active this Article?');"><span class="label label-danger">Pending</span></a>
              <?php }else{ ?>
                <a href="<?php echo base_url().'backend/users/publish_users/no/'.$id; ?>" onclick="return confirm('Do you want to nonactive this Article');"><span class="label label-info">Active</span></a>                    
              <?php } ?>
						</td>
						<td>
							<a href="<?php echo base_url().'backend/users/update_users/'.$id; ?>"><button class="btn btn-warning btn-xs">Edit</button></a>
							<a href="<?php echo base_url().'backend/users/delete_data_users/'.$id; ?>" onclick="return confirm('Do you want to delete this row?')"><button class="btn btn-danger btn-xs">Delete</button></a>
						</td>
					</tr>
					<?php 
							} 
